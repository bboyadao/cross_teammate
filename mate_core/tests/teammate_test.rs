...
async fn test_teammate_creation() {
    ...
    let tm = Teammate::anew_from_expenses(expenses).await;
...
async fn test_calculate_empty() {
    let tm = Teammate::anew_from_expenses(vec![]).await;
...
async fn test_calculate_single_participant() {
    ...
    let tm = Teammate::anew_from_expenses(vec![exp("Solo", 500)]).await;
    assert!(tm.calculate().is_empty());
}
